...
public interface HospitalMedicalInfoRepo extends JpaRepository<HospitalMedicalInfo, String> {
	...
	@Query("SELECT h FROM HospitalMedicalInfo h WHERE " + "CASE WHEN :bloodType = 'A+' THEN h.countAPlus > 0 "
			+ "     WHEN :bloodType = 'A-' THEN h.countAMin  > 0 " +

			"     WHEN :bloodType = 'O-' THEN h.countOMin  > 0 " + "     WHEN :bloodType = 'O+' THEN h.countOPlus  > 0 "
			+

			"     WHEN :bloodType = 'B-' THEN h.countBMin  > 0 " + "     WHEN :bloodType = 'B+' THEN h.countBPlus  > 0 "
			+
